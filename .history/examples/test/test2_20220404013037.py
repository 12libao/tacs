...
# ml = pyamg.smoothed_aggregation_solver(A, B, max_levels=200, max_coarse=100,keep=True)
# [asa, work] = pyamg.aggregation.adaptive_sa_solver(A, num_candidates=1, max_coarse=100, max_levels=200)
...
resi = []
x = ml.solve(b=b, tol=1e-12, residuals=resi, accel='cg')
...
print((resi[-1]/residuals[0])**(1.0/(len(residuals)-1)))
