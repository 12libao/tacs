...
ml = pyamg.smoothed_aggregation_solver(A, B, max_levels=100, max_coarse=100,keep=True)
# [asa, work] = pyamg.aggregation.adaptive_sa_solver(A, num_candidates=1)
# print(asa)
print(ml)
...
# x = asa.solve(b=np.ones((A.shape[0],)), x0=np.ones((A.shape[0],)), residuals=residuals, accel='cg')
x = ml.solve(b=b, x0=x0, tol=1e-15, residuals=residuals, accel='cg')
